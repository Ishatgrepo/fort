...
async def run(bot, message):
    ...
    # Select a bot (cycle through available bots based on active tasks)
    active_tasks = temp.lock.get(user_id, {})
    available_bots = [b for b in bots if b['id'] not in [task['bot']['id'] for task in active_tasks.values()]]
    if not available_bots:
        return await message.reply("All your bots are currently in use. Wait for a task to complete or add more bots.")
    bot_choice = available_bots[0]  # Use the first available bot
